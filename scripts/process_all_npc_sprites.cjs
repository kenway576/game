...
const characters = [
  {
    src: '1788479821479.jpg',
    target: 'npc_kenta.webp',
    name: '健太 (Kenta - Cheerful Athletic Boy)',
    yCut: 0
  },
  {
    src: '1788479888344.jpg',
    target: 'npc_hiroki.webp',
    name: '宏树 (Hiroki - Glasses Honor Student)',
    yCut: 1098 // remove ground cobblestone texture
  },
  {
    src: '1788479939547.jpg',
    target: 'npc_aoi.webp',
    name: '葵 (Aoi - Energetic Classmate Girl)',
    yCut: 0
  },
  {
    src: '1788480009353.jpg',
    target: 'npc_shiori.webp',
    name: '诗织 (Shiori - Shy Literary Girl)',
    yCut: 0
  },
  {
    src: '1788480060526.jpg',
    target: 'npc_fujiwara.webp',
    name: '藤原老师 (Fujiwara-sensei - Homeroom & Japanese Teacher)',
    yCut: 0
  },
  {
    src: '1788480104992.jpg',
    target: 'npc_sakamoto.webp',
    name: '坂本老师 (Sakamoto-sensei - PE Teacher & Dean)',
    yCut: 0
  },
  {
    src: '1788480183681.jpg',
    target: 'npc_kanzaki.webp',
    name: '神崎老师 (Kanzaki-sensei - Science & Chemistry Teacher)',
    yCut: 0
  },
  {
    src: '1788480231084.jpg',
    target: 'npc_ellen.webp',
    name: '艾伦老师 (Ellen-sensei - Foreign English Teacher)',
    yCut: 0
  },
  {
    src: '1788480269735.jpg',
    target: 'npc_yamada.webp',
    name: '山田阿姨 (Yamada-san - Cafeteria Cook Lady)',
    yCut: 0
  },
  {
    src: '1788480311673.jpg',
    target: 'npc_saeki.webp',
    name: '佐伯校医 (Saeki-sensei - School Nurse)',
    yCut: 0
  }
];
...
async function cleanSprite(item) {
  ...
  // Handle bottom ground cut if specified
  if (item.yCut > 0) {
    for (let y = item.yCut; y < H; y++) {
      for (let x = 0; x < W; x++) {
        bg[y * W + x] = 1;
      }
    }
  ...
  for (let p0 = 0; p0 < N; p0++) {
    ...
    if (cells.length >= 60 && cells.length <= N * 0.15) {
      let pureCount = 0;
...
async function run() {
  ...
  .jpeg({ quality: 92 })
  .toFile(path.join(srcDir, '_audit_10_npcs_cutout.jpg'));

  console.log('\nAll 10 NPC character sprites processed, defringed, cropped and saved!');
  console.log('Saved audit sheet: _audit_10_npcs_cutout.jpg');
}
