...
const DialogueBox: React.FC<Props> = ({ character, pages, vocabulary, onFinish }) => {
  ...
  useEffect(() => {
    if (!step) return;
    setTyping(true); setDispN(''); setDispS(''); setShowVocab(false);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-3 md:gap-5 animate-in fade-in slide-in-from-bottom-4">
      {step.narration && (
        <div className="w-11/12 md:w-5/6 bg-slate-900/60 backdrop-blur-sm border border-blue-400/20 rounded-lg p-3 md:p-5 shadow-xl text-center cursor-pointer" onClick={next}>
          {/* 🔥 使用 dangerouslySetInnerHTML 以渲染 Ruby 标签 */}
      ...
      <div className={`relative w-full bg-black/85 backdrop-blur-2xl border-t-4 border-white/10 p-6 md:p-12 shadow-2xl rounded-t-xl transition-all ${!step.speech ? 'h-0 overflow-hidden opacity-0' : 'opacity-100'}`}>
        {step.speech && (
          <div className="cursor-pointer" onClick={next}>
            ...
            <div className={`absolute -top-7 md:-top-10 left-6 md:left-12 px-8 md:px-12 py-2 ${character.color} text-white font-black italic text-lg md:text-2xl shadow-2xl transform -skew-x-12 border-2 border-white/20`}>
        ...
        {!typing && idx === steps.length - 1 && vocabulary?.length > 0 && (
          <div className="mt-8 pt-6 border-t border-white/10">
            <div className="flex justify-between items-center mb-4">
              <span className="text-xs text-yellow-500 font-black tracking-widest uppercase">Vocabulary / 新出単語</span>
              <button onClick={() => setShowVocab(!showVocab)} className="bg-yellow-600/30 text-yellow-400 border border-yellow-500/50 px-4 py-1.5 rounded-full text-xs font-black uppercase transition-all">{showVocab ? 'Hide' : 'Show'}</button>
            </div>
            {showVocab && (
        ...
        {!typing && <div className="absolute right-8 bottom-6 animate-bounce text-yellow-400 text-2xl md:text-4xl">▼</div>}
      </div>
    </div>
  );
};
